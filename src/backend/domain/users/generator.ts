import { createXorShift32, type RNG } from '../shared/rng'
...
function pickDutchTussenvoegsel(rng: RNG): string | '' {
  ...
  return pick(opts as string[], rng)
}
...
function makeDutchFullName(rng: RNG): string {
  ...
  const compoundChance = rng.nextInt(100) < 20
  if (compoundChance) {
    let second = pickDutchFirstName(rng)
    if (second === first) second = pickDutchFirstName(rng)
    first = `${first}-${second}`
  ...
  const clean = (s: string) =>
    s.replace(/^(van\s+de\s+|van\s+der\s+|van\s+|de\s+|den\s+|ter\s+)/i, '')
  last1 = clean(last1)
